...
final class GenUiBackendServerConfig {
  const GenUiBackendServerConfig({
    ...
    this.modelSha256,
    this.bearerToken,
    this.cachePolicy = ModelCachePolicy.preferCached,
  ...
  factory GenUiBackendServerConfig.fromEnvironment(
    ...
    return GenUiBackendServerConfig(
      modelSource: ModelSource.parse(_expandHome(rawModelSource, environment)),
      port: _parseInt(
      ...
      modelSourceDisplayName:
          ...
          (rawModelSource == defaultModelSource
              ...
              : null) ??
          ModelSource.parse(rawModelSource).displayName,
      cacheDirectory:
  ...
  ModelLoadOptions loadOptionsFor(ModelSource source) {
    if (source.isLocal) {
      return ModelLoadOptions(sha256: modelSha256);
    }
...
ModelCachePolicy _parseCachePolicy(String? value) {
  if (value == null || value.trim().isEmpty) {
    return ModelCachePolicy.preferCached;
  }
  return switch (value.trim().toLowerCase()) {
    'prefercached' ||
    'prefer_cached' ||
    'prefer-cached' => ModelCachePolicy.preferCached,
    'refresh' => ModelCachePolicy.refresh,
    'cacheonly' || 'cache_only' || 'cache-only' => ModelCachePolicy.cacheOnly,
    'nocache' || 'no_cache' || 'no-cache' => ModelCachePolicy.noCache,
