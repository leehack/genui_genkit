...
The skipped profile benchmark compares local inference parameter sets for one
selected model:

```bash
flutter test test/local_inference_profile_benchmark_test.dart \
  --dart-define=GENUI_RUN_LOCAL_BENCHMARK=true
```

Use `GENUI_BENCHMARK_PROFILE` to run only profiles whose names match a regular
expression. Add `-d <device-id>` to run the same explicit benchmark target on a
connected Flutter device.
