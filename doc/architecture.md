...
- The root `genui_genkit` package owns the Flutter session, backend interfaces,
  provider-neutral Genkit adapter, remote flow adapter, hybrid router, and
  lightweight widgets.
- On-device llamadart model resolution, cache/download progress, warm-up, and
  lifecycle helpers belong to `genkit_llamadart`. Apps can pass its prepared
  model refs into `GenkitBackend`.
- `example/flutter_hybrid_genui` demonstrates a product-like client app with
  Local, Gemini, and Backend routes.
- `example/genui_backend_server` demonstrates backend mode with
...
- [ADR 0001: Keep Llamadart Model Lifecycle APIs In genkit_llamadart](decisions/0001-llamadart-integration-boundary.md)
