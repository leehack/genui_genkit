...
final class AppRuntime {
  ...
  factory AppRuntime.fromEnvironment(Map<String, String> environment) {
    ...
    final backend = HybridGenUiBackend(
      routes: {
        GenUiAiRoute.local.metadataValue: LlamaLocalGenkitBackend(
          config.localModel,
    ...
    final session = GenkitGenUiSession(
      ...
      metadataBuilder: () => {
        ...
        'localModelSource': config.localModel.modelSource.metadataSourceKey,
    ...
    return AppRuntime(
      ...
      modelStatus: modelStatus,
      prepareModel:
          (backend.routes[GenUiAiRoute.local.metadataValue]!
                  as LlamaLocalGenkitBackend)
              .prepare,
      dispose: () {
        session.dispose();
        modelStatus.dispose();
